/**
 * OpenRecruitOS Community Edition — Demo data seeder
...
async function main() {
  console.log("Seeding OpenRecruitOS demo data...");
  ...
  const applications: { id: string; stage: string; candidateIdx: number; jobIdx: number }[] = [];
  for (const [ci, ji, stage, applied, moves] of plan) {
    ...
    applications.push({ id: app.id, stage, candidateIdx: ci, jobIdx: ji });
  }
  console.log(`✓ ${applications.length} applications created with stage history`);
  ...
  console.log(`✓ ${interviewSpecs.length} interviews created`);
